...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["register_submit_button"]) && !$USER->IsAuthorized())
{
	...
	else // if there;s no any errors - create user
	{
		...
		if ($bOk)
		{
			...
			// записать свойства в инфоблок
			$el = new CIBlockElement;
			$PROP = array();
			//$PROP["type"] =  Array("VALUE" => intval($_REQUEST["USERTYPE"]));   
			//$PROP["status"] = "066438062a7d1d949d38e84fb179f628";   // не проверен 
//			$PROP["user"] = $user;
	//		$PROP["director_name"] = $_REQUEST["director_name"];   
	//		$PROP["juridical_address"] = $_REQUEST["juridical_address"];   
	//		$PROP["postal_address"] = $_REQUEST["postal_address"];   
	//		$PROP["delivery_address"] = $_REQUEST["delivery_address"];   
	//		$PROP["phone"] = $_REQUEST["phone"];   
	//		$PROP["contact_person_name"] = $_REQUEST["contact_person_name"];   
	//		$PROP["contact_person_mail"] = $_REQUEST["contact_person_mail"];  
	//		$PROP["contact_person_ICQ"] = $_REQUEST["contact_person_ICQ"];   
	//		$PROP["contact_person_phone"] = $_REQUEST["contact_person_phone"];   
